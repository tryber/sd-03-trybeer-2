...
const CardrDetailsOrders = ({ orderDetails }) => 
  orderDetails.map((e, idx) => {
    ...
    return (
      <div key={ randomNumber() }>
        ...
        <p data-testid={ `${idx}-product-total-value` }>{` R$ ${e['salePrice'].toFixed(2).toString().replace('.', ',')}`}</p>
        <p
          data-testid="order-total-value"
        >
          {`Total: R$ ${e['totalSalePrice'].toFixed(2).toString().replace('.', ',')}`}
        </p>
      </div>
    )};
  });
